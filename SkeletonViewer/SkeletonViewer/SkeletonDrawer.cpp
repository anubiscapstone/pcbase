...
bool SkeletonDrawer::parseMessage(String^ msg, RECT bounds)
{
	try
	{
		...
		double scaley = ((bounds.bottom - bounds.top) / 2.0);
		double midx = scalex + bounds.left;
		double midy = scaley + bounds.top;
		midx += OFFSET_X * scalex;
		midy += OFFSET_Y * scaley;
		scalex *=  SCALE;
		scaley *= -SCALE;

		Head[0]					 = (Double::Parse(jointParts[0])  * scalex)	+ midx		;
		Head[1]					 = (Double::Parse(jointParts[1])  * scaley)	+ midy		;
		ShoulderCenter[0]		 = (Double::Parse(jointParts[2])  * scalex)	+ midx		;
		ShoulderCenter[1]		 = (Double::Parse(jointParts[3])  * scaley)	+ midy		;
		ShoulderLeft[0]			 = (Double::Parse(jointParts[4])  * scalex)	+ midx		;
		ShoulderLeft[1]			 = (Double::Parse(jointParts[5])  * scaley)	+ midy		;
		ShoulderRight[0]		 = (Double::Parse(jointParts[6])  * scalex)	+ midx		;
		ShoulderRight[1]		 = (Double::Parse(jointParts[7])  * scaley)	+ midy		;
		ElbowLeft[0]			 = (Double::Parse(jointParts[8])  * scalex)	+ midx		;
		ElbowLeft[1]			 = (Double::Parse(jointParts[9])  * scaley)	+ midy		;
		ElbowRight[0]			 = (Double::Parse(jointParts[10]) * scalex)	+ midx		;
		ElbowRight[1]			 = (Double::Parse(jointParts[11]) * scaley)	+ midy		;
		WristLeft[0]			 = (Double::Parse(jointParts[12]) * scalex)	+ midx		;
		WristLeft[1]			 = (Double::Parse(jointParts[13]) * scaley)	+ midy		;
		WristRight[0]			 = (Double::Parse(jointParts[14]) * scalex)	+ midx		;
		WristRight[1]			 = (Double::Parse(jointParts[15]) * scaley)	+ midy		;
		HandLeft[0]				 = (Double::Parse(jointParts[16]) * scalex)	+ midx		;
		HandLeft[1]				 = (Double::Parse(jointParts[17]) * scaley)	+ midy		;
		HandRight[0]			 = (Double::Parse(jointParts[18]) * scalex)	+ midx		;
		HandRight[1]			 = (Double::Parse(jointParts[19]) * scaley)	+ midy		;
		Spine[0]				 = (Double::Parse(jointParts[20]) * scalex)	+ midx		;
		Spine[1]				 = (Double::Parse(jointParts[21]) * scaley) + midy		;
		HipCenter[0]			 = (Double::Parse(jointParts[22]) * scalex)	+ midx		;
		HipCenter[1]			 = (Double::Parse(jointParts[23]) * scaley)	+ midy		;
		HipLeft[0]				 = (Double::Parse(jointParts[24]) * scalex)	+ midx		;
		HipLeft[1]				 = (Double::Parse(jointParts[25]) * scaley)	+ midy		;
		HipRight[0]				 = (Double::Parse(jointParts[26]) * scalex)	+ midx		;
		HipRight[1]				 = (Double::Parse(jointParts[27]) * scaley)	+ midy		;
		KneeLeft[0]				 = (Double::Parse(jointParts[28]) * scalex)	+ midx		;
		KneeLeft[1]				 = (Double::Parse(jointParts[29]) * scaley)	+ midy		;
		KneeRight[0]			 = (Double::Parse(jointParts[30]) * scalex)	+ midx		;
		KneeRight[1]			 = (Double::Parse(jointParts[31]) * scaley)	+ midy		;
		AnkleLeft[0]			 = (Double::Parse(jointParts[32]) * scalex)	+ midx		;
		AnkleLeft[1]			 = (Double::Parse(jointParts[33]) * scaley)	+ midy		;
		AnkleRight[0]			 = (Double::Parse(jointParts[34]) * scalex)	+ midx		;
		AnkleRight[1]			 = (Double::Parse(jointParts[35]) * scaley)	+ midy		;
		FootLeft[0]				 = (Double::Parse(jointParts[36]) * scalex)	+ midx		;
		FootLeft[1]				 = (Double::Parse(jointParts[37]) * scaley)	+ midy		;
		FootRight[0]			 = (Double::Parse(jointParts[38]) * scalex)	+ midx		;
		FootRight[1]			 = (Double::Parse(jointParts[39]) * scaley)	+ midy		;
	}
	catch (Exception^ e)
	{
		return false;
